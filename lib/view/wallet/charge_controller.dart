import 'dart:convert';
import 'dart:io';
import 'package:flutter/material.dart';
import 'package:flutter_application_restaurant/core/static/routes.dart';
import 'package:flutter_application_restaurant/main.dart';
import 'package:flutter_application_restaurant/navigation_bar.dart';
import 'package:get/get.dart';
import 'package:http/http.dart' as http;
import 'package:image_picker/image_picker.dart';

class ChargeController extends GetxController {
  final Rx<File?> proofImage = Rx<File?>(null);
  final RxString transferMethod = ''.obs;
  final RxString amount = ''.obs;
  final RxString note = ''.obs;
  final RxBool isLoading = false.obs;

  final ImagePicker _picker = ImagePicker();

  final String apiUrl = '${Linkapi.backUrl}/wallet/charge';
  final String userToken = '$token';

  Future<void> pickProofImage() async {
    final XFile? image = await _picker.pickImage(source: ImageSource.gallery);
    if (image != null) {
      proofImage.value = File(image.path);
    }
  }


  Future<void> submitChargeRequest() async {
  if (proofImage.value == null || transferMethod.isEmpty ) {
    Get.snackbar('Alert', 'Please fill in all required fields');
    return;
  }

  isLoading.value = true;
  try {
    final uri = Uri.parse(apiUrl);
    var request = http.MultipartRequest('POST', uri)
      ..headers['Authorization'] = 'Bearer $userToken'
      ..fields['transfer_method'] = transferMethod.value
      ..fields['amount'] = amount.value
      ..fields['note'] = note.value
      ..files.add(await http.MultipartFile.fromPath(
        'proof_image',
        proofImage.value!.path,
      ));

    var streamedResponse = await request.send();
    var response = await http.Response.fromStream(streamedResponse); 

    print('نجاااح Response from backend: ${response.body}');

    if (response.statusCode == 201) {
      final Map<String, dynamic> decodedData = json.decode(response.body);
      final String successMessage = decodedData['message'] ?? 'تم إرسال طلب الشحن بنجاح.';
      Get.snackbar(
        'Success',
        successMessage, 
        snackPosition: SnackPosition.BOTTOM,
        backgroundColor: Colors.green,
      );
      Get.to(MyHomePageScreen());
    } else {
      Get.snackbar(
        'Alert',
        ' ${response.statusCode}',
        snackPosition: SnackPosition.BOTTOM,
        backgroundColor: Colors.red[500],
      );
    }
  } catch (e) {
    Get.snackbar(
      'Alert',
      ' $e',
      snackPosition: SnackPosition.BOTTOM,
      backgroundColor: Colors.red[500],

    );
  } finally {
    isLoading.value = false;
  }
}}